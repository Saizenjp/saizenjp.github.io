...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'frontdesk.html': '<h4>이 화면이 하는 일</h4>실시간 <b>도착·출발·재실</b> + 팀별 방번호·잔액·메모 통합 현황. 프론트=바=레스토랑 한 화면(테이블 관리는 미도입 — 명패는 계속 출력).<h4>계산·판정</h4><ul><li>🛬체크인=<code>dep===오늘</code> / 🛫체크아웃=<code>arr===오늘</code> / 🏨재실(연박)=그 사이 / 🍽석식=그날 묵는 전원.</li><li>잔액=청구−결제 합산. <b>KPI 클릭→해당 라인 스크롤</b>.</li><li><b>숙소 칩</b>으로 리스트·KPI 필터.</li></ul><h4>바로가기</h4>팀 클릭 → 상세에서 <b>정산</b>(그 팀 자동 열기)·<b>방배정</b>(그 날짜)·메모로 점프.<h4>권한·데이터</h4>room 영역. 읽기 집계(데이터 변경 없음).',

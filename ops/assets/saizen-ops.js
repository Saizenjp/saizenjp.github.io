...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      ix_cHkCd: r('日付','ひづけ')+'ごと・'+r('階','かい')+r('単位','たんい')+'の'+r('清掃','せいそう')+r('割当表','わりあてひょう')+'。'+r('退室','たいしつ')+'／'+r('連泊','れんぱく')+'／'+r('入室','にゅうしつ')+'・'+r('滞在','たいざい')+r('期間','きかん')+'・'+r('作業量','さぎょうりょう')+'（'+r('閲覧','えつらん')+'のみ）',
    ...
    ko: {
      ...
      ix_cHkH: '객실 청소',
      ix_cHkCd: '날짜별·층 단위 청소 배정표. 퇴실/연박/입실·체류 기간·예상 작업량 (보기 전용)',
      ix_cOccupH: '일별 체류 현황',
    ...
    en: {
      ...
      ix_cHkCd: 'Daily worksheet by floor — departures / stayovers / arrivals, stay period, workload (read-only)',
  ...
  var SO_HELP = {
    ...
    'housekeeping.html': '<h4>이 화면이 하는 일</h4>객실 청소팀이 <b>그날 어느 방을 어떤 순서로</b> 치우는지 봅니다. 방배정은 프론트 사무실이 하고 이 화면은 <b>보기 전용</b>입니다(여기서는 아무것도 바뀌지 않습니다).<h4>상태 5종</h4><ul><li><b>턴오버(퇴실→입실)</b> — 오늘 나가고 오늘 또 들어옵니다. <b>가장 먼저</b> 치워야 체크인 시각을 지킵니다.</li><li><b>퇴실</b> — 오늘 나갑니다(오늘 밤 다음 손님 없음).</li><li><b>연박</b> — 계속 묵습니다. 정비만 합니다.</li><li><b>입실</b> — 비어 있던 방에 오늘 들어옵니다. 점검·세팅.</li><li><b>공실 / 폐쇄</b> — 비어 있음 / 기간 폐쇄된 방.</li></ul><h4>순서</h4>기본 정렬은 <b>턴오버 → 퇴실 → 연박 → 입실 → 공실</b>입니다. 연박은 <b>티오프가 이른 방부터</b> 놓습니다 — 팀이 라운딩을 나간 뒤라야 방에 들어갈 수 있기 때문입니다. 「호수순」으로 바꾸면 방 번호 순서로 봅니다.<h4>표 읽는 법</h4><ul><li>왼쪽 색띠 = 상태. 층 머리줄을 누르면 그 층을 접습니다.</li><li><b>박째</b> = 오늘까지 묵은 밤 수 / 전체 박수. <b>시트 교체</b>는 3박째마다 뜹니다.</li><li><b>티오프</b> = 그날 그 팀의 가장 이른 라운딩 시각과 코스.</li><li><b>비고</b> = 시트 교체 · 입실 인원 · 마지막밤 · 폐쇄 사유 · 객실 메모.</li></ul><h4>작업량</h4>퇴실 30분 · 연박 15분 · 공실 점검 5분으로 잡아 합계를 내고, 1인 6시간 기준으로 필요 인원을 환산합니다. 층 머리줄에도 그 층 몫이 나옵니다. 기준을 바꾸려면 페이지 위쪽 상수(MIN_OUT·MIN_STAY·MIN_VACANT·SHIFT_MIN·LINEN_EVERY)를 고칩니다.<h4>인쇄</h4>층마다 한 장으로 A4 세로 인쇄됩니다.<h4>권한·데이터</h4>hk 영역. room_inventory·rooms·guests·bookings·room_closures·golf_groups 읽기만 합니다.',
    ...
    'cart.html': '<h4>이 화면이 하는 일</h4><b>전기카트 관리표</b>입니다. 위는 <b>월 캘린더</b>(날짜마다 팀 수·보유·사용·잔여), 아래는 <b>고른 날짜의 신청팀 목록</b>입니다. 캘린더 칸을 누르면 아래가 그 날짜로 바뀝니다.<h4>지금은 전기카트만</h4>가솔린 2인승·4인승은 화면에서 <b>빼둔 상태</b>입니다(Min 2026-08). 저장된 데이터는 그대로라 언제든 되살릴 수 있습니다.<h4>자동 분배</h4>메리트투어가 <b>현지전달비고에 「전기카트 신청」</b>만 적어 보내면 날짜는 시스템이 채웁니다 — <b>인천 출발팀=입국일부터</b>, <b>부산 출발팀=귀국일 포함</b>, 체류 중인 날은 전원. 대수는 <b>4명당 1대</b>. <b>간지호텔은 평일이 구주고원CC</b>라 전기카트가 안 되므로 그 날은 자동으로 빠집니다(야마나미CC 쓰는 날만 잡힘). 화면은 저장 전에도 규칙대로 채워져 보이고(「자동」 배지), <b>[이 날 자동 분배]·[이 달 자동 분배]</b>로 확정 저장합니다.<h4>보유 대수</h4>블럭표의 <b>보유</b> 칸을 그 자리에서 고칩니다. <b>정비·고장으로 못 쓰는 카트가 있으면 보유 수를 줄이면</b> 그만큼 잔여에서 빠집니다. 카트 번호는 <b>1번부터 보유 대수까지</b> 팀 순서대로 자동 배분돼 표에 표시됩니다(팀마다 번호를 적을 필요 없음).<h4>목록·수배서</h4>기본은 <b>그날 전기카트 신청·배정 팀만</b> 나옵니다. 현장 즉석 신청을 받으려면 <b>「그날 전체 팀」</b>을 체크해 그날 라운딩 팀 전부를 펼칩니다. 팀 이름 옆 <b>「수배서」</b>를 누르면 現地手配書가 팝업으로 열립니다. <b>코스·티오프</b>는 골프 조편성에서 저장한 값을 읽어옵니다.<h4>요금</h4>전기카트 <b>1대 1일 ¥2,000</b>. 가솔린은 0(기본 제공). 행별·전체 합계가 자동 계산됩니다.<h4>권한·데이터</h4>golf 영역(읽기는 로그인 전체). cart_types·cart_bookings 읽기·쓰기 / bookings·guests·passengers·golf_groups 읽기. ⚠ <code>93_cart.sql</code> 실행 필요(94는 개별 카트용이었으나 현재 화면 미사용).',

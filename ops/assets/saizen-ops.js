...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'qrcards.html': '<h4>이 화면이 하는 일</h4>체크인 때 손님에게 드리는 <b>팀별 QR 카드</b>를 인쇄합니다(手配書와 별도, 손님 전용 소품). 손님이 폰으로 찍어 두면, 주문 때 POS에서 그 QR을 스캔해 팀이 자동 선택됩니다.<h4>쓰는 법</h4><ul><li>월(◀▶) 선택 → 그날 <b>入国日·숙소 칩</b>으로 대상 팀을 좁힙니다(체크인 팀만 뽑기 좋음).</li><li><b>[🔳 손님 QR카드 인쇄]</b> → A4에 팀별 미니 카드(이름+QR+안내문) 여러 장. 잘라서 체크인 때 보여주면 손님이 촬영.</li><li>토큰은 팀 고유 랜덤값 — 처음 열 때 자동 발급(발급은 print/room 권한).</li></ul><h4>주의</h4>카드엔 <b>이름·QR·안내문만</b> 담겨 다른 손님 정보는 없습니다. QR 없이 온 손님은 POS에서 검색 후 <b>「이 팀 맞습니까?」</b> 확인으로 처리합니다.',

...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'frontdesk.html': '<h4>이 화면이 하는 일</h4>실시간 <b>도착·출발·재실</b> + 팀별 방번호·잔액·메모 통합 현황. 프론트=바=레스토랑 한 화면(테이블 관리는 미도입 — 명패는 계속 출력).<h4>계산·판정</h4><ul><li>🛬체크인=<code>dep===오늘</code> / 🛫체크아웃=<code>arr===오늘</code> / 🏨체류중(연박)=그 사이 / 🍽석식=그날 묵는 전원.</li><li>잔액=청구−결제 합산. <b>KPI 클릭→해당 라인 스크롤</b>.</li><li><b>숙소 칩</b>으로 리스트·KPI 필터.</li></ul><h4>운영 상태 한눈에</h4>팀 상세에 각 섹션 입력이 배지로 모입니다 — <b>💎회원(등급) · 🛫조기퇴실 · 🍽夕食除外 · ✂석식분리 · 🔗운영팀 · 🗝키택발급 · 🔔확인필요</b>(💎은 팀 내 회원 수·등급을 서열 없이 라벨 그대로)(🔔은 클릭 시 해당 처리화면으로). 여러 화면을 안 돌아도 한 팀의 상태를 한 곳에서 확인.<h4>비고·메모 인라인 편집</h4>팀 클릭 → 상세에서 <b>비고(운영)·메모를 그 자리에서 바로 입력</b>(입력칸을 벗어나면 자동 저장, 변경이력 기록). 별도 메모 페이지에 들어갈 필요 없이 일하는 화면에서 남기고, 現地手配書·재실 현황과 같은 event_notes를 공유합니다.<h4>바로가기</h4>팀 클릭 → 상세에서 <b>정산</b>(그 팀 자동 열기)·<b>방배정</b>(그 날짜)로 점프.<h4>🔔 확인 필요(후속 조치)</h4>조기 퇴실 등으로 <b>다른 메뉴에 후속 작업</b>(예: 정산 환불)이 생기면 상단 패널에 모입니다. <b>[처리하기]</b>로 해당 화면(딥링크)으로 바로 가고, 끝내면 <b>[완료]</b>로 닫습니다. 미완료 N건은 랜딩 <b>🔔 배지</b>에도 떠서 어느 PC에서든 보입니다. 로그인한 담당자 누구나 완료할 수 있습니다.<h4>권한·데이터</h4>front(프론트 데스크) 영역. 읽기 집계(데이터 변경 없음) · 확인 필요(followups)는 로그인 전원 읽기/완료.',

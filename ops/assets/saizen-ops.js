...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'dispatch.html': '<h4>이 화면이 하는 일</h4>행사별 <b>A4 양면</b> — 앞=現地手配書, 뒤=現地発生分 記入表.<h4>주문 QR</h4>手配書마다 <b>팀 고유 주문 QR</b>이 자동 발급·인쇄됩니다(팀 우측 상단). 손님이 체크인 때 이 QR을 촬영해 두고, 주문 시 POS에서 스캔하면 그 팀이 자동 선택됩니다. QR은 추측 불가한 랜덤 토큰이라 안전.<h4>계산·판정</h4><ul><li>라운딩 일정 자동·部屋数=<code>ceil(pax/2)</code>室.</li><li><b>마스킹 토글</b>: ON=생년월일까지 / OFF=여권·전화 노출.</li><li>記入表(뒷면)=현장 손기입, B2B 정산과 별개.</li></ul><h4>권한·데이터</h4>print(인쇄) 영역.',

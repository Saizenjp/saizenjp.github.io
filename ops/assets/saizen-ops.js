...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      s1_recodeLbl: 'Fコード 再発行 基準日（例外作業）',
      s1_recodeHint: 'この日付以降に出発する一般チーム（Fコード）のうち、今回のファイルに含まれるものだけを新しいコードで付け直します。空欄＝付け直さない（通常）。⚠ 印刷物を出した分は入れないでください。',
      s1_recodeLog: '⚠ Fコード再発行: {d} 以降 出発の {n}チーム のコードを解放し、新ルールで付け直します。',
    ...
    ko: {
      ...
      s1_h2conn: '① 데이터베이스 연결',
      s1_recodeLbl: 'F코드 재발급 기준일 (예외 작업)',
      s1_recodeHint: '이 날짜 이후 출발하는 일반고객 팀(F코드) 중 이번 파일에 들어있는 것만 새 코드로 다시 부여합니다. 비워두면 다시 부여하지 않습니다(평소). ⚠ 인쇄물이 이미 나간 구간은 넣지 마세요.',
      s1_recodeLog: '⚠ F코드 재발급: {d} 이후 출발 {n}팀의 코드를 풀고 새 규칙으로 다시 부여합니다.',
      s1_lblUrl: 'Project URL',
      s1_lblKey: 'anon key',
    ...
    en: {
      ...
      s1_recodeLbl: 'F-code reissue cutoff (exception)',
      s1_recodeHint: 'Reissues codes only for non-member teams (F codes) departing on or after this date AND present in this upload. Leave empty to keep all codes (normal). ⚠ Do not cover dates whose printouts already went out.',
      s1_recodeLog: '⚠ F-code reissue: released codes for {n} team(s) departing on/after {d}; reassigning with the new rule.',
      s1_lblUrl: 'Project URL',
      s1_lblKey: 'anon key',

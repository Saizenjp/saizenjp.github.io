...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'aircover.html': '<h4>A5 <b>세로</b> · 골프백에 붙여 쓰는 종이</h4>현장이 이 종이를 <b>골프백에 테이프로 붙여</b> 놓고 옮기면서 봅니다. 그래서 손으로 쓸 때 크게 쓰던 순서 그대로 배치했습니다 — 위에서부터 <b>태그코드 → 출발지(釜山·ソウル, 색) → 체류기간(한 줄) → 番·名</b>, 항공편·시각은 보조 정보라 아래에 작게(<b>施設 줄은 제거</b> — 백에 붙은 종이에선 안 쓰여서). 그만큼 태그·출발지·기간·番名을 키웠습니다. 체류기간은 <b>入国/帰国을 두 줄로 흩지 않고 <code>08/09 – 08/13</code> 한 줄</b>로 묶었습니다. 태그코드는 글자수에 맞춰 자동 축소해 항상 한 줄입니다. <b>A5 세로, 배율 100%</b>로 인쇄하세요.<h4>묶음 팀 = 조건이 맞으면 1장</h4>手配書에서 <b>묶은 팀</b>이 <b>출국일·귀국일·출국편·귀국편·출발지(ICN/PUS)까지 전부 같으면</b>, 실질적으로 한 팀이 예약만 나뉜 것이므로 <b>航空カバー를 1장으로 합칩니다</b> — 태그코드는 팀번호 없이 기준코드 그대로(<code>DRな-Y</code>), <b>인원은 합산</b>(4팀 14명 → 「14名 1〜14番」). 하나라도 다르면 커버에 적을 내용이 달라지므로 <b>기존대로 Y1·Y2…로 나눠</b> 출력합니다. ※네임택은 무관하게 개인번호 1·2·3·4로 계속 나뉩니다.<h4>이 화면이 하는 일</h4>팀별 <b>A5 항공커버</b> 1장(가로). 개인 항공편·시설색.<h4>계산·판정</h4><ul><li>대표=<code>is_rep</code> 우선.</li><li><b>시설색·라벨</b>=태그 끝글자(Y/K/G/S).</li><li>항공편 ZE→PUS·TW→ICN 보정.</li><li>태그·인원 <b>인라인 수정</b>=print_overrides 공유 → 석식과 동기.</li></ul><h4>주요 용어</h4>태그코드(3자리, F접두=비회원).<h4>권한·데이터</h4>print(인쇄) 영역. 태그·인원 수정은 print 또는 room 권한 필요.',

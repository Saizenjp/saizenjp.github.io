...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      po_qrScan:'📷 QR'+r('読取','よみとり'), po_qrTitle:'お'+r('客様','きゃくさま')+'の'+r('注文','ちゅうもん')+'QRを'+r('読取','よみとり'), po_qrManualPh:'QR'+r('読取','よみとり')+'できない'+r('時','とき')+'はコード'+r('手入力','てにゅうりょく'), po_qrManualGo:r('検索','けんさく'),
    ...
    ko: {
      ...
      po_qrScan:'📷 QR 스캔', po_qrTitle:'손님 주문 QR 스캔', po_qrManualPh:'QR 안 될 때 코드 수기입력', po_qrManualGo:'검색',
      po_active: '주문중',
      po_secTeam: '팀',
      po_secMember: '일행',
      po_checkin: '체크인',
    ...
    en: {
      ...
      po_qrScan:'📷 Scan QR', po_qrTitle:'Scan guest order QR', po_qrManualPh:'Enter code if QR fails', po_qrManualGo:'Find',
      po_active: 'ordering',
      po_secTeam: 'Teams',
  ...
  var SO_HELP = {
    ...
    'dispatch.html': '<h4>이 화면이 하는 일</h4>행사별 <b>A4 양면</b> — 앞=現地手配書, 뒤=現地発生分 記入表.<h4>주문 QR 카드</h4><b>[🔳 주문 QR카드]</b> 버튼으로 <b>팀별 미니 QR 카드</b>(이름+QR·안내문만)를 인쇄합니다. 체크인 때 손님이 폰으로 찍어 두고, 주문 시 POS에서 스캔하면 그 팀이 자동 선택됩니다. 手配書 본문엔 QR을 넣지 않아(다른 정보 노출 없음) 카드만 손님에게 배부하면 됩니다. 토큰은 추측 불가한 랜덤값.<h4>계산·판정</h4><ul><li>라운딩 일정 자동·部屋数=<code>ceil(pax/2)</code>室.</li><li><b>마스킹 토글</b>: ON=생년월일까지 / OFF=여권·전화 노출.</li><li>記入表(뒷면)=현장 손기입, B2B 정산과 별개.</li></ul><h4>권한·데이터</h4>print(인쇄) 영역.',

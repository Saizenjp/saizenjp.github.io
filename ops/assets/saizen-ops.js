...
(function (global) {
  ...
  var SO_HELP = {
    ...
    'dinner.html': '<h4>이 화면이 하는 일</h4>날짜별 <b>夕食オーダー</b>(A3) + 조·중·석 <b>식수 자동집계</b> + <b>レストラン名札</b> 인쇄.<h4>계산·판정</h4><ul><li>식수=숙소 그룹별 규칙. <b>석식=그날 묵는 전원</b>.</li><li><b>조기 퇴실 반영</b>: 방배정에서 🛫 조기 퇴실 처리한 인원은 끼니 경계대로 자동 차감 — <b>퇴실일 아침까지는 포함</b>, 점심·저녁부터 제외. 인쇄 명단·합계에 <code>早期退室 −N</code> 표기.</li><li><b>팀 묶기(🔗)</b>: 합석할 팀을 체크 → 「선택 묶기」. <b>レストラン名札이 1장으로 합쳐</b>지고 대표자·태그코드가 모두 표기·인원은 합산(칸은 자동 축소). 식수 집계는 그대로(각 팀 인원 유지). 「묶음 해제」로 되돌림.</li><li><b>제외(병합)</b>: 한 팀 지우고 다른 팀에 인원 취합 → 명단·식수에서 빠짐(묶기와 달리 명단에서 사라짐).</li><li><b>분배 정합성</b>: 원래 합계=수정 후 합계인지 ✓/⚠.</li></ul><h4>別注 → 정산 자동청구</h4>단가가 있는 <b>추가·업그레이드 別注를 등록하면 그 팀 御請求書(정산)에 자동으로 청구</b>가 잡힙니다(업그레이드=차액, 알레르기=청구 없음). 별주를 수정·삭제하면 청구도 자동으로 따라갑니다(DB 트리거). 팀 정산 계정이 없으면 자동 개설됩니다.<h4>주요 용어</h4>태그코드. 「팀별 태그·인원」은 항공커버와 공유(夕食만 제외 반영) · 「묶기」는 명패 전용.<h4>권한·데이터</h4>print(인쇄) 영역. 태그·인원·묶기 저장은 print 또는 room 권한 필요. 別注 청구는 charges(트리거가 생성, 정산은 settle/pos 영역에서 열람).',
    'shizu.html': '<h4>이 화면이 하는 일</h4><b>志津の宿 予約表</b>(시즈노야도 료칸에 보내는 객실 예약표)를 출력합니다. <b>방배정(room) 화면에서 한 시즈노야도 배정을 그대로 읽어</b> 날짜별 7객실(本館 4·別棟 3) 예약표를 자동 생성합니다.<h4>계산·판정</h4><ul><li><b>배정은 방배정에서</b>: 이 화면은 읽어서 출력만 — 방을 바꾸려면 방배정(room) 화면에서 합니다.</li><li><b>連泊·합계·男女</b>는 배정·명단에서 자동(성별은 일행별예약 기준, 이름으로 매칭).</li><li><b>別棟(吉祥·瑞雲·馬酔木) 온천 사전신청</b>: 別棟 배정 인원에 ♨ 토글 → 사전신청한 인·박만 <b>×¥2,000</b> 집계(객실료 ¥17,000은 本館·別棟 동일). 신청 시에만 현지에서 온천물을 준비.</li></ul><h4>출력</h4>화면 미리보기 + <b>予約表 xlsx</b>(템플릿 서식) + 印刷. 숫자는 전각(全角) 표기.<h4>권한·데이터</h4>print(인쇄) 영역. rooms·guest_members·passengers 읽기 + shizu_onsen(온천 사전신청, print/room 쓰기).',

...
(function (global) {
  ...
  'dispatch.html': '<h4>이 화면이 하는 일</h4>행사별 <b>A4 양면</b> — 앞=現地手配書, 뒤=現地発生分 記入表.<h4>주문 QR 카드는 별도 페이지</h4>손님에게 주는 <b>주문 QR카드</b>는 랜딩 인쇄물의 <b>「주문 QR카드」</b> 카드(전용 페이지)에서 인쇄합니다. 手配書 본문엔 QR을 넣지 않습니다(다른 정보 노출 방지). 토큰은 手配書를 열 때 팀별로 자동 발급됩니다.<h4>계산·판정</h4><ul><li>라운딩 일정 자동·部屋数=<code>ceil(pax/2)</code>室.</li><li><b>마스킹 토글</b>: ON=생년월일까지 / OFF=여권·전화 노출.</li><li>記入表(뒷면)=현장 손기입, B2B 정산과 별개.</li><li><b>참고사항1·참고사항2 열</b>: 예약리스트의 <b>비고</b>(참고사항1)·<b>현지비고</b>(참고사항2)를 목록에 표시. <b>참고사항2</b>(特食·온천·별관 등 <b>현장 필수</b>)는 빨강 강조. <b>現地메모</b>는 운영팀이 직접 쓰는 별도 메모(구분).</li><li><b>「팀:A,B」 자동 묶기</b>: 예약 비고에 「팀:…」이 있으면 등록(step1) 때 그 대표명으로 팀을 찾아 手配書·夕食에 자동 묶음. 한쪽 비고에만 적혀도 됨. 수동 묶기는 보존.</li></ul><h4>권한·데이터</h4>print(인쇄) 영역.',
    ...
    'shizu.html': '<h4>이 화면이 하는 일</h4><b>志津の宿 予約表</b>(시즈노야도 료칸 객실 예약표)를 만들고 <b>객실 배정 보드</b>로 개인 단위 배정합니다. 7객실=本館 4(志津·合歓·北條·山法師)·別棟 3(吉祥·瑞雲·馬酔木).<h4>배정</h4><ul><li><b>객실 배정 보드</b>: 왼쪽 미배정에서 인원 선택 → 오른쪽 객실 클릭으로 배정. 이름 클릭=이동·✕=미배정·드래그도 가능. <b>검색</b>(이름·영문명·대표자·행사번호)으로 찾아 그 날짜로 점프.</li><li><b>🪄 월별 자동배정</b>: 표시 중인 <b>그 달만</b> 배정(다른 달 무영향). <b>2인 페어·빈 방만</b> 채우고 홀수 1명·3인은 미배정으로 둠(수기 보완). <b>순서 — 별채 선호팀은 別棟(吉祥·瑞雲·馬酔木) 먼저→本館, 일반팀은 本館(志津·合歓·山法師) 먼저→別棟. 北條는 두 경우 모두 항상 <u>맨 마지막</u>에 배정</b>(예비·비선호 객실).</li><li><b>정원 2 + 강제 1명</b>: 수기로 최대 3인까지. 3인 방은 「3/2 超過」로 붉게 표시.</li><li><b>번들예약</b>(야마나미+시즈 한 예약)은 예약기간 전체가 아니라 <b>실제 시즈 숙박 구간만</b> 미배정(未) 계산(현지비고 「M/D-M/D 시즈」 파싱·배정된 방 기준).</li></ul><h4>메모·표기</h4><ul><li><b>参考事項</b>=메리트 현지비고(읽기·DeepL 일본어 번역). <b>現地메모</b>=방 칸 클릭해 직접 입력(팀 단위·자동 저장).</li><li><b>♨ 별채(내탕)</b>: 현지비고에 별채신청·내탕신청·온천신청 등 키워드가 있으면 別棟 방에 ♨ 자동 표기. 사전신청=<b>+2,000엔/인·박=19,000엔</b>(별채 3방·메리트 B2B). ※미신청 팀의 현장 물채우기(+4,000엔/방·박)는 별도.</li><li>날짜 칩에 <b>入N/出N</b>(체크인/체크아웃 팀)·<b>未N</b>(미배정). 월 선택↔보드 사이에 그날 <b>체크인/체크아웃 팀 요약</b>.</li></ul><h4>출력</h4>화면 예약표를 <b>그대로 인쇄</b>(팝업 없음·2일/장·큰 글씨) + <b>予約表 xlsx</b>·날짜별 인쇄. 全角 표기.<h4>권한·데이터</h4><b>shizu 영역</b>(admin.html에서 지정). rooms·guest_members·passengers 읽기 + shizu_team_memo(現地메모)·shizu_memo 쓰기(shizu/room 영역).',
  ...
  var SO_HELP_VER = { 'dispatch.html': 3, 'shizu.html': 4 };

...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      ix_cSignageH: r('案内','あんない')+'モニター · 2F'+r('夕食','ゆうしょく'), ix_cSignageCd: '2Fエレベーター前のテレビに「本日の夕食」を全画面表示（会場・時間・チーム）。会場は夕食オーダー画面で指定',
      ix_cSignageOfficeH: r('運営','うんえい')+r('状況','じょうきょう')+'モニター · スタッフ', ix_cSignageOfficeCd: '事務所・フロント裏のテレビに本日の進行(チェックイン到着確認・チェックアウト・部屋未割当・厨房・送迎・夕食・ゴルフ)を数字で。30秒更新、名前・部屋番号・金額なし',
      ix_cSignageCourseH: r('案内','あんない')+'モニター · コース', ix_cSignageCourseCd: 'コース入口・クラブハウスのテレビに本日の組合せ表(ティ時刻×3コース)を全画面表示。ログイン不要、17時以降は翌日',
      ix_cSignageLobbyH: r('案内','あんない')+'モニター · ロビー', ix_cSignageLobbyCd: 'ロビーのテレビに 歓迎板・本日の組合せ表・ご出発案内(便・ホテル出発時刻)・夕食・天気・お知らせ を順に表示。お知らせは部署掲示板で「モニターにも表示」',
    ...
    ko: {
      ...
      ix_gAdmin: '관리 · 마스터',
      ix_gMgmt: '경영',
      ix_cStayCalH: '일별 체류 현황', ix_cStayCalCd: '날짜별 체류 인원·체크인/아웃 캘린더 (금액 없음 · 이 화면만 따로 권한 부여 가능)',
      ix_cStatsH: '경영 통계',
      ...
      ix_cSignageH: '안내 모니터 · 2층 석식', ix_cSignageCd: '2층 엘리베이터 앞 TV에 「오늘 석식」 전체화면 (장소·시간·팀). 장소는 석식 오더 화면에서 지정',
      ix_cSignageOfficeH: '운영 현황 모니터 · 직원용', ix_cSignageOfficeCd: '사무실·프론트 뒤 TV에 오늘 진행(체크인 도착 확인·체크아웃·방 미배정·주방·송영·석식·골프)을 숫자로. 30초 갱신, 이름·방번호·금액 없음',
      ix_cSignageCourseH: '안내 모니터 · 코스', ix_cSignageCourseCd: '코스 입구·클럽하우스 TV에 오늘 조편성표(티오프×3코스) 전체화면. 로그인 불필요, 17시 이후엔 내일',
      ix_cSignageLobbyH: '안내 모니터 · 로비', ix_cSignageLobbyCd: '로비 TV에 환영판·오늘 코스 배정표·출발 안내(항공편·호텔 출발시각)·석식·날씨·공지를 차례로 표시. 공지는 부서 보드에서 「모니터에도 표시」',
    ...
    en: {
      ...
      ix_cSignageH: 'Signage · 2F dinner', ix_cSignageCd: 'Full-screen “tonight’s dinner” for the 2F elevator TV (venue, time, teams). Set the venue on the Dinner Order page',
      ix_cSignageOfficeH: 'Ops status monitor · Staff', ix_cSignageOfficeCd: 'Back-office / behind-front TV: today’s progress in numbers (check-in arrivals, check-outs, unassigned rooms, kitchen, transfers, dinner, golf). 30-second refresh; no names, room numbers or money',
      ix_cSignageCourseH: 'Signage · Course', ix_cSignageCourseCd: 'Full-screen tee sheet (tee time × 3 courses) for the course entrance / clubhouse TV. No login; switches to tomorrow after 17:00',
      ix_cSignageLobbyH: 'Signage · Lobby', ix_cSignageLobbyCd: 'Lobby TV rotation: welcome board, today’s tee sheet, departures (flight, hotel departure time), dinner, weather, notices. Mark notices “show on monitor” on the board',

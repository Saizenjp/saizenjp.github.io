...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      ix_posGolfCd: r('手袋','てぶくろ')+'・ボール・'+r('帽子','ぼうし')+'など'+r('物品','ぶっぴん')+r('販売','はんばい')+'のみ。9'+r('ホール','')+r('追加','ついか')+'は'+r('フロント','')+'で'+r('請求','せいきゅう')+'します。',
      ...
      ix_cHkCd: r('日付','ひづけ')+'ごと・'+r('階','かい')+r('単位','たんい')+'の'+r('清掃','せいそう')+r('割当表','わりあてひょう')+'。'+r('退室','たいしつ')+'／'+r('連泊','れんぱく')+'／'+r('入室','にゅうしつ')+'・'+r('滞在','たいざい')+r('期間','きかん')+'（'+r('閲覧','えつらん')+'のみ）',
      ix_cOccupH: r('日別','ひべつ')+' '+r('滞在','たいざい')+r('状況','じょうきょう'),
      ix_cOccupCd: r('日付','ひづけ')+r('別','べつ')+'に'+r('使用','しよう')+'／'+r('保有','ほゆう')+r('室数','しつすう')+'を'+r('一覧','いちらん')+'（'+r('複数月','ふくすうげつ')+r('連続','れんぞく')+'）',
      ...
      ix_cCartCd: r('日付','ひづけ')+r('別','べつ')+'の'+r('保有','ほゆう')+'・'+r('使用','しよう')+'・'+r('残数','ざんすう')+'とチーム'+r('別','べつ')+r('割当','わりあて')+'（コース・ティ'+r('時刻','じこく')+'）',
      ix_cCourseH: r('コース','')+r('割当表','わりあてひょう'),
      ix_cCourseCd: r('阿蘇','あそ')+'·'+r('祖母','そぼ')+'·'+r('久住','くじゅう')+'と'+r('ティ','')+r('時刻','じこく')+'（6:50〜8:42・7'+r('分','ふん')+r('間隔','かんかく')+'）'+r('割当','わりあて')+' + '+r('組編成','くみへんせい')+'（'+r('誰','だれ')+'がどの'+r('組','くみ')+'か）を1'+r('画面','がめん')+'で。'+r('自動','じどう')+r('配分','はいぶん')+'·'+r('除外','じょがい')+'·A4'+r('横','よこ')+r('印刷','いんさつ'),
      ...
      ix_cTransferH: '送迎表', ix_cTransferCd: '空港ピックアップ・お送りを便ごとに · 人数点呼チェック表',
    ...
    ko: {
      ...
      ix_posGolf: '골프샵 POS',
      ix_posGolfCd: '장갑·볼·모자 등 물품 판매만 합니다. 9홀 추가는 프론트에서 청구합니다.',
      ...
      ix_cHkH: '객실 청소',
      ix_cHkCd: '날짜별·층 단위 청소 배정표. 퇴실/연박/입실·체류 기간 (보기 전용)',
      ix_cOccupH: '일별 체류 현황',
      ix_cOccupCd: '날짜별 사용/보유 객실 수를 한 표로(여러 달 이어보기) · 칸 클릭 시 그날 팀 목록',
      ix_cKeyslipH: '룸키 라벨',
      ...
      ix_cCartH: '전기카트 배정표',
      ix_cCartCd: '날짜별 보유·사용·잔여 + 팀별 배정(코스·티오프) · 전기카트 사전/현장 신청',
      ix_cCourseH: '코스 배정표',
      ix_cCourseCd: '아소·소보·쿠주와 티오프(6:50~8:42·7분 간격) 배정 + 조편성(누가 어느 조)까지 한 화면에서. 자동 배분·제외·A4 가로 인쇄',
      ix_cGolfH: '골프 조편성',
      ix_cGolfCd: '상품명 기반 라운딩 날짜 자동 → 4인 1조 편성(아소·소보·쿠주)·티오프·조편성표 인쇄',
      ix_cTransferH: '송영표', ix_cTransferCd: '공항 픽업·샌딩을 항공편별로 · 인원 점검 체크표',
    ...
    en: {
      ...
      ix_posGolfCd: 'Shop sales only — gloves, balls, caps. Extra 9 holes are charged at the front desk.',
      ...
      ix_cHkCd: 'Daily worksheet by floor — departures / stayovers / arrivals, stay period (read-only)',
      ix_cOccupH: 'Daily occupancy',
      ix_cOccupCd: 'Rooms in use vs stock by date across several months · click a cell for that night\'s teams',
      ...
      ix_cCartCd: 'Daily stock / in use / remaining and per-team assignment (course, tee time)',
      ix_cCourseH: 'Course assignment',
      ix_cCourseCd: 'Courses (Aso·Sobo·Kuju) and tee times (6:50–8:42, 7-min) plus the group roster, on one screen. Auto-balance, skip, A4 landscape print',
      ix_cGolfH: 'Golf Grouping',
      ix_cGolfCd: 'Round days auto-derived from product → form 4-somes (Aso·Sobo·Kuju), tee times, print sheet',
      ix_cTransferH: 'Transfer sheet', ix_cTransferCd: 'Airport pickups and drop-offs by flight · head-count checklist',
  ...
  var SO_HELP = {
    ...
    'occupancy.html': '<h4>이 화면이 하는 일</h4>날짜별로 <b>객실이 얼마나 찼는지</b>를 한 표로 봅니다. 메리트투어 「호텔 블록」 표는 <b>잔여</b>를 보는 용도지만, 현장은 <b>채워진 수량</b>을 봐야 하므로 방향을 뒤집었습니다. <b>읽기 전용</b>(데이터 변경 없음).<h4>표 읽는 법</h4><ul><li>행 = <b>숙소 → 룸타입</b>, 열 = 날짜. 여러 달을 골라 <b>이어서</b> 볼 수 있습니다(달이 바뀌는 열에 세로선).</li><li>칸 <b>위 = 그날 밤 사용 객실 수</b>, <b>아래 = 보유 객실 수</b>. 색은 잔여 기준 — 초록(4실+)·노랑(1~3실)·빨강(만실·초과).</li><li>체류 판정 = <code>check_in ≤ 날짜 &lt; check_out</code>(퇴실일 밤은 비어 있음). 같은 객실에 여러 명이 있어도 <b>1실</b>로 셉니다.</li><li>맨 아래 <b>합계</b> 행 = 보이는 숙소들의 그날 총 사용 실수.</li></ul><h4>칸을 누르면</h4>그날 그 룸타입에 들어 있는 <b>팀 목록</b>(방번호·그룹코드·대표·인원·체류기간)이 아래에 나옵니다.<h4>권한·데이터</h4>room 영역. room_inventory·rooms·guests·bookings 읽기.',
    ...
    'transfer.html': '<h4>이 화면이 하는 일</h4>그날 <b>공항 픽업(도착)</b>·<b>샌딩(출발)</b> 하는 팀을 <b>항공편별</b>로 묶어 <b>송영표(점호 체크표)</b>를 인쇄합니다. 기사·인솔 담당이 종이 한 장 들고 인원을 확인하는 용도입니다.<h4>무엇이 나오나</h4><ul><li><b>이름 명단은 넣지 않습니다</b> — 팀 코드·대표자·<b>인원</b>·행선지(숙소)·<b>항공편명</b>·<b>출발지 → 도착지</b>·시각만 나옵니다.</li><li><b>도착</b> = 그날 입국(현지 도착)하는 팀. 시각은 <b>현지 도착시각</b>, 구간은 출발지 → 도착 공항.</li><li><b>출발</b> = 그날 귀국하는 팀. 시각은 <b>현지 출발시각</b>, 구간은 출발 공항 → 도착지.</li><li>같은 팀이라도 <b>인천·부산이 섞이면 편별로 줄이 나뉘고</b> 각 줄의 인원이 따로 셉니다(명단의 개인별 항공정보 기준).</li><li>명단이 아직 안 들어온 팀은 <b>예약 인원</b>으로 한 줄 나옵니다(편명·시각 빈칸).</li></ul><h4>체크 방법</h4>행마다 <b>확인 네모칸</b>과 <b>실인원 기입칸</b>이 있습니다. 인원이 예약과 다르면 실인원에 적고, 편별 <b>소계</b>로 차량 인원을 맞춥니다. 상단에 담당자·차량·출발시각 기입칸이 있습니다.<h4>화면</h4>날짜(◀ ▶ · 오늘) · 구분(전체/도착/출발) · 숙소 칩으로 거른 뒤 <b>[🖨 송영표 인쇄]</b>. 인쇄는 A4 세로·흑백입니다.<h4>권한·데이터</h4>print 영역. bookings·guests·passengers 읽기만 합니다(변경 없음).',
  ...
  'dispatch.html': '<h4>이 화면이 하는 일</h4>행사별 <b>A4 양면</b> — 앞=現地手配書, 뒤=現地発生分 記入表.<h4>주문 QR 카드는 별도 페이지</h4>손님에게 주는 <b>주문 QR카드</b>는 랜딩 인쇄물의 <b>「주문 QR카드」</b> 카드(전용 페이지)에서 인쇄합니다. 手配書 본문엔 QR을 넣지 않습니다(다른 정보 노출 방지). 토큰은 手配書를 열 때 팀별로 자동 발급됩니다.<h4>거르기</h4>출발일 · 숙소 · <b>출발지(전체 / ICN 인천 / PUS 부산·김해)</b> 칩으로 목록을 좁힙니다. 거른 결과가 <b>一括印刷</b>에도 그대로 적용되니 인천 팀만 따로 뽑을 수 있습니다. ※인별 출발지가 섞인 팀은 ICN·PUS 양쪽에 모두 나옵니다(빠지지 않게).<h4>계산·판정</h4><ul><li>라운딩 일정 자동·部屋数=<code>ceil(pax/2)</code>室.</li><li><b>마스킹 토글</b>: ON=생년월일까지 / OFF=여권·전화 노출.</li><li>記入表(뒷면)=현장 손기입, B2B 정산과 별개.</li><li><b>EVカート·룸차지 자동 반영</b>: 엠클릭 <b>현지 비고</b>(비고·참고사항 포함)에 「전기카트 2대」·「룸 업그레이드」처럼 적혀 있으면 → <b>앞면</b>에 「EVカート 申請」 전용 섹션으로 강조, <b>뒷면 記入表</b>엔 해당 区分(①룸차지·②EV카트)에 ○가 찍히고 内容이 미리 채워집니다(금액·수량·담당은 현장 손기입). 적힌 게 없으면 앞면은 「申請なし」, 뒷면은 빈 양식 그대로.</li><li><b>참고사항1·참고사항2 열</b>: 예약리스트의 <b>비고</b>(참고사항1)·<b>현지비고</b>(참고사항2)를 목록에 표시. <b>참고사항2</b>(特食·온천·별관 등 <b>현장 필수</b>)는 빨강 강조. <b>現地메모</b>는 운영팀이 직접 쓰는 별도 메모(구분).</li><li><b>「팀:A,B」 자동 묶기</b>: 예약 비고에 「팀:…」이 있으면 등록(step1) 때 그 대표명으로 팀을 찾아 手配書·夕食에 자동 묶음. 한쪽 비고에만 적혀도 됨. 수동 묶기는 보존.</li></ul><h4>권한·데이터</h4>print(인쇄) 영역.',
    ...
    'stats.html': '<h4>이 화면이 하는 일</h4>대표·부서장용 <b>월/연 통합 통계</b>. 기간(이번 달·올해·작년·최근 12개월·범위)을 고르면 입도·매출·가동률·고객구성을 한 화면에 집계합니다. <b>읽기 전용</b>(데이터 변경 없음).<h4>계산·판정</h4><ul><li><b>입도(送客)</b>=현지 체크인(<code>dep_date</code>) 기준·인원=<b>예약 인원</b>. 숙소별·출발지(ICN/PUS)별 분해.</li><li><b>B2B 매출</b>=인원×박수×숙소단가+인원×¥6,000(야마나미·쿠주 14,000 / 간지 16,000 / 시즈 17,000).</li><li><b>현장 매출</b>=charges(취소 제외·JST 기준), <b>현금/카드/미지정</b> 분리 + 구분별.</li><li><b>회원 비율</b>=member_grade·member_class·member_div 3컬럼 OR(하나라도 회원이면 회원), 등급별.</li><li><b>객실 가동률</b>=사용 침대-박 ÷ (가동 객실 정원×기간 일수). 체크인 월 귀속 <b>근사치</b>.</li></ul><h4>손익 구간(가동률 몇 %면 흑자인가)</h4>상단 <b>손익 구간</b> 패널이 <b>자체(야마나미) 객실 가동률</b>과 <b>손익분기선</b>을 나란히 보여줍니다. <b>손익분기 = 가동률 75.9%</b>(자체 침대 248 × 영업일 275일 기준)이고, 그 아래면 적자·위면 흑자입니다. <b>가동률 1%p = 연 822만엔</b>이라, 1% 떨어지면 그만큼 그대로 이익이 줍니다. 표는 60~95% 구간별 <b>연 매출·연 영업이익</b>이며 지금 위치와 손익분기 행에 배지가 붙습니다. ⚠ <b>매출 단가는 실적(¥15,154/인박)</b>이지만 <b>변동비 ¥3,100·연 고정비 6.24억엔(인건비·코스관리·수도광열·판관·감가상각)은 추정</b>입니다 — 실제 급여·광열·코스 지출이 확정되면 <code>saizen-core.js</code>의 <code>RESORT_PL</code> 상수만 고치면 화면이 따라옵니다. 근거·산정 과정은 <code>docs/profitability-estimate.md</code>.<h4>📅 캘린더 보기</h4>상단 <b>[📅 캘린더]</b>로 <b>월/연 체류 현황</b>을 봅니다. 월 보기 = 한 칸이 하루, 큰 숫자가 <b>그날 밤 묵는 인원</b>이고 그 옆에 <b>팀 수</b>, 아래에 🛬체크인·🛫체크아웃(팀·인원)이 붙습니다. 색이 진할수록 인원이 많고, 날짜를 누르면 그날 <b>프론트 데스크</b>로 갑니다. 연 보기 = 월별 연인박·팀수·최다 인원. <b>예약 기준</b>이라 방배정 전 팀도 포함됩니다(홈 화면 「지금 체류」 띠와 같은 기준).<h4>주요 용어</h4>B2B(메리트 선계약)와 현장 매출은 <b>별개</b>. 가동률은 침대(정원) 기준.<h4>권한·데이터</h4><b>stats 영역</b> — admin 또는 stats 지정자만(매니저 자동통과 아님). 집계 RPC <code>exec_stats</code>(서버 합산, security definer).',

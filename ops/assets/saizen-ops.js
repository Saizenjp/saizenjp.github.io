...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      ix_gSign: r('案内','あんない')+'モニター', ix_gSignDesc: 'テレビ用の全画面 · ログイン不要 · 自動更新 · 新しいウィンドウで開く',
      ix_cInvHkH: '清掃・リネン在庫',
      ...
      ix_c5H: r('現場','げんば')+r('精算','せいさん'),
      ix_c5Cd: '滞在中の追加料金を集計 · チェックアウト時に御請求書を発行',

      ix_posRest: 'レストラン・'+r('コンベンションホール','')+' POS',
      ix_posRestCd: '飲食・コンベンションホールの注文をチームのご精算に · 厨房分は厨房へ',
      ix_posFront: 'フロント POS',
      ix_posFrontCd: 'フロント販売品と追加料金をご精算に反映',
      ix_posGolf: 'ゴルフ場 POS',
      ix_posGolfCd: 'プレー料金（9H追加・カート）と物販。フロントからも売り場タブで入って会計できます',
      ...
      ix_gMgmt: r('経営','けいえい'),
      ix_cStayCalH: r('滞在','たいざい')+'カレンダー', ix_cStayCalCd: '日別の滞在人数・チェックイン/アウト（金額なし・この画面だけ権限付与可）',
      ix_cStatsH: r('経営','けいえい')+r('統計','とうけい'),
      ix_cStatsCd: '入込・売上・稼働率・客層 · 予約推移・滞在カレンダー',
      ...
      ix_cWatchCd: '規定から外れた予約形態の点検',


      ix_cGroup: 'グループコード・'+r('会員','かいいん')+'マスター',
      ix_cGroupCd: '会員グループコードの登録・照会 · コード未保有会員の割当',
      ...
      ix_cSignageH: '2F '+r('夕食','ゆうしょく')+r('画面','がめん'), ix_cSignageCd: '「本日の夕食」の会場・時間・チームを全画面（会場は夕食オーダーで指定）',
      ix_cSignageOfficeH: 'スタッフ'+r('用','よう')+' '+r('運営','うんえい')+r('状況','じょうきょう'), ix_cSignageOfficeCd: '本日の進行を数字で（チェックイン・アウト・部屋未割当・厨房・送迎・夕食・ゴルフ）· 名前・部屋番号・金額なし',
      ix_cSignageCourseH: 'コース'+r('組合','くみあわ')+'せ'+r('画面','がめん'), ix_cSignageCourseCd: '本日の組合せ表（ティ時刻×3コース）を全画面 · 17時以降は翌日',
      ix_cSignageLobbyH: 'ロビー'+r('画面','がめん'), ix_cSignageLobbyCd: '歓迎板 · 組合せ · ご出発案内 · 夕食 · 天気 · お知らせ を順に表示（お知らせは部署掲示板で「モニター表示」）',
      ...
      ix_dFrontDesc: r('部屋','へや')+r('割','わ')+'り・フロント・POS・'+r('精算','せいさん'),
      ...
      bd_doPin: ''+r('固定','こてい'), bd_guest: 'ロビー'+r('案内','あんない')+'モニターにも'+r('表示','ひょうじ')+'（お客様向け・日韓併記で）', bd_guestBadge: 'モニター', bd_guestOn: 'モニター表示', bd_guestOff: 'モニター解除',
      bd_del: r('削除','さくじょ'),
    ...
    ko: {
      ...
      ix_gSign: '안내 모니터', ix_gSignDesc: 'TV용 전체화면 · 로그인 불필요 · 자동 갱신 · 새 창으로 열림',
      ix_cInvHkH: '청소 · 린넨 재고',
      ...
      ix_posRest: '레스토랑·컨벤션 홀 POS',
      ix_posRestCd: '식음·컨벤션 홀 주문을 팀 계산에 · 주방행은 주방으로',
      ix_posFront: '프론트 POS',
      ix_posFrontCd: '프론트 판매 물품과 추가 요금을 계산에 반영',
      ix_posGolf: '골프장 POS',
      ix_posGolfCd: '플레이 요금(9홀 추가·카트)과 물품 판매. 프론트에서도 매장 탭으로 들어와 계산합니다',
      ...
      ix_gAdmin: '관리 · 마스터',
      ix_gMgmt: '경영',
      ix_cStayCalH: '체류 캘린더', ix_cStayCalCd: '날짜별 체류 인원·체크인/아웃 캘린더 (금액 없음 · 이 화면만 따로 권한 부여 가능)',
      ix_cStatsH: '경영 통계',
      ix_cStatsCd: '입도·매출·가동률·고객 구성 · 예약 추이·체류 캘린더',
      ix_cVisitorH: '방문 통계',
      ...
      ix_cWatch: '특이 예약 감시',
      ix_cWatchCd: '규정을 벗어난 예약 형태 점검',


      ix_cGroup: '그룹코드 · 회원 마스터',
      ix_cGroupCd: '회원 그룹코드 등록·조회 · 코드 미보유 회원 배정',
      ix_annTag: '공지',
      ix_boardOpen: '보드 열기 →',
      ix_fuTag: '🔔 확인 필요',
      ix_fuOpen: '프론트에서 처리 →',
      ...
      ix_cSignageH: '2층 석식 화면', ix_cSignageCd: '「오늘 석식」 장소·시간·팀을 전체화면으로 (장소는 석식 오더에서 지정)',
      ix_cSignageOfficeH: '직원용 운영 현황', ix_cSignageOfficeCd: '오늘 진행을 숫자로 (체크인·체크아웃·방 미배정·주방·송영·석식·골프) · 이름·방번호·금액 없음',
      ix_cSignageCourseH: '코스 조편성 화면', ix_cSignageCourseCd: '오늘 조편성표(티오프×3코스)를 전체화면으로 · 17시 이후엔 내일',
      ix_cSignageLobbyH: '로비 화면', ix_cSignageLobbyCd: '환영판 · 조편성 · 출발 안내 · 석식 · 날씨 · 공지를 차례로 표시 (공지는 부서 보드에서 「모니터 표시」)',
      ...
      ix_dFront: '프론트·객실',
      ix_dFrontDesc: '방배정 · 프론트 · POS · 정산',
      ix_dFnb: '식음 (F&B)',
      ix_dFnbDesc: 'POS · 주방 · 메뉴',
      ix_dGolf: '골프',
      ix_dGolfDesc: '코스 · 조편성 · 카트 배정',
      ...
      bd_unpin: '고정 해제',
      bd_doPin: '고정', bd_guest: '로비 안내 모니터에도 표시 (손님용 · 한·일 병기로)', bd_guestBadge: '모니터', bd_guestOn: '모니터 표시', bd_guestOff: '모니터 해제',
      bd_del: '삭제',
      bd_today: '오늘 요약',
      bd_ciTeam: '체크인 팀',
      bd_coTeam: '체크아웃 팀',
    ...
    en: {
      ...
      ix_gSign: 'Signage monitors', ix_gSignDesc: 'Full-screen TV pages · no login · auto-refresh · opens in a new window',
      ...
      ix_posGolf: 'Golf course POS',
      ix_posGolfCd: 'Play fees (extra 9H, carts) and retail. The front desk can switch here to bill them',
      ...
      ix_gMgmt: 'Management',
      ix_cStayCalH: 'Stay calendar', ix_cStayCalCd: 'Daily in-house counts and check-ins/outs (no money · can be granted on its own)',
      ix_cStatsH: 'Executive Stats',
      ix_cStatsCd: 'Visitors, revenue, occupancy and guest mix · booking trend and stay calendar',
      ...
      ix_cWatchCd: 'Review bookings that fall outside policy',


      ix_cGroup: 'Group Codes · Members',
      ix_cGroupCd: 'Register and look up member group codes; assign codes to members without one',
      ix_annTag: 'Notice',
      ...
      ix_cSignageH: '2F dinner screen', ix_cSignageCd: 'Tonight’s dinner venue, time and teams full screen (set the venue on Dinner Order)',
      ix_cSignageOfficeH: 'Staff ops status', ix_cSignageOfficeCd: 'Today’s progress in numbers (check-in/out, unassigned rooms, kitchen, transfers, dinner, golf) · no names, rooms or money',
      ix_cSignageCourseH: 'Course tee sheet screen', ix_cSignageCourseCd: 'Today’s tee sheet (tee time × 3 courses) full screen · switches to tomorrow after 17:00',
      ix_cSignageLobbyH: 'Lobby screen', ix_cSignageLobbyCd: 'Rotates welcome board · tee sheet · departures · dinner · weather · notices (mark notices “show on monitor” on the board)',
      ...
      ix_dFrontDesc: 'Rooms · front · POS · settlement',
      ix_dFnb: 'F&B',
      ix_dFnbDesc: 'POS · Kitchen · Menu',
      ix_dGolf: 'Golf',
      ix_dGolfDesc: 'Course · grouping · carts',
      ...
      bd_unpin: 'Unpin',
      bd_doPin: 'Pin', bd_guest: 'Also show on the lobby signage (guest-facing, write in Korean + Japanese)', bd_guestBadge: 'Monitor', bd_guestOn: 'Show on monitor', bd_guestOff: 'Remove from monitor',
      bd_del: 'Delete',
  ...
  function guardPage() {
    ...
    meAccess().then(function (acc) {
      ...
      // 세부 영역 트리(SZCore.AREA_TREE): 부모 키 = 그룹 전체, 자식 키 = 그 화면만. 페이지는 자식 키를 선언한다.
      if (global.SZCore && SZCore.areaAllowed) {
        var lv = SZCore.areaAllowed(acc, areas);
        if (lv === 'write') return;
        if (lv === 'read') { showReadOnly(); return; }
  ...
  'dispatch.html': '<h4>이 화면이 하는 일</h4>행사별 <b>A4 양면</b> — 앞=現地手配書, 뒤=現地発生分 記入表.<h4>손님 QR 카드는 별도 페이지</h4>손님에게 드리는 <b>손님 QR카드</b>는 랜딩 인쇄물의 <b>「손님 QR카드」</b> 카드(전용 페이지)에서 인쇄합니다. 手配書 본문엔 QR을 넣지 않습니다(다른 정보 노출 방지). 토큰은 手配書를 열 때 팀별로 자동 발급됩니다.<h4>거르기</h4>출발일 · 숙소 · <b>출발지(전체 / ICN 인천 / PUS 부산·김해)</b> 칩으로 목록을 좁힙니다. 거른 결과가 <b>一括印刷</b>에도 그대로 적용되니 인천 팀만 따로 뽑을 수 있습니다. ※인별 출발지가 섞인 팀은 ICN·PUS 양쪽에 모두 나옵니다(빠지지 않게).<h4>계산·판정</h4><ul><li>라운딩 일정 자동·部屋数=<code>ceil(pax/2)</code>室.</li><li><b>마스킹 토글</b>: ON=생년월일까지 / OFF=여권·전화 노출.</li><li>記入表(뒷면)=현장 손기입, B2B 정산과 별개.</li><li><b>EVカート·룸차지 자동 반영</b>: 엠클릭 <b>현지 비고</b>(비고·참고사항 포함)에 「전기카트 2대」·「룸 업그레이드」처럼 적혀 있으면 → <b>앞면</b>에 「EVカート 申請」 전용 섹션으로 강조, <b>뒷면 記入表</b>엔 해당 区分(①룸차지·②EV카트)에 ○가 찍히고 内容이 미리 채워집니다(금액·수량·담당은 현장 손기입). 적힌 게 없으면 앞면은 「申請なし」, 뒷면은 빈 양식 그대로.</li><li><b>참고사항1·참고사항2 열</b>: 예약리스트의 <b>비고</b>(참고사항1)·<b>현지비고</b>(참고사항2)를 목록에 표시. <b>참고사항2</b>(特食·온천·별관 등 <b>현장 필수</b>)는 빨강 강조. <b>現地메모</b>는 운영팀이 직접 쓰는 별도 메모(구분).</li><li><b>「팀:A,B」 자동 묶기</b>: 예약 비고에 「팀:…」이 있으면 등록(step1) 때 그 대표명으로 팀을 찾아 手配書·夕食에 자동 묶음. 한쪽 비고에만 적혀도 됨. 수동 묶기는 보존.</li></ul><h4>한 팀만 다시 뽑기</h4>카드를 잃어버렸으면 <b>[재발급]</b>을 누르세요 — 새 QR로 갱신되고(이전 QR은 그 즉시 무효) 이어서 <b>그 팀 카드만</b> 인쇄할지 물어봅니다. 한 장이라도 잃어버리면 어차피 재발급해야 하므로, 같은 QR을 그냥 다시 뽑는 버튼은 두지 않았습니다.<h4>팀 묶음</h4>現地手配書에서 묶은 팀은 <b>대표팀 코드+팀번호</b>(<code>DFな-Y1</code>·<code>DFな-Y2</code>)로 표시됩니다 — 手配書·航空カバー·네임택·룸키 라벨과 같은 코드입니다.<h4>권한·데이터</h4>print(인쇄) 영역.',
    'dinner.html': '<h4>이 화면이 하는 일</h4>날짜별 <b>夕食オーダー</b>(A3) + 조·중·석 <b>식수 자동집계</b> + <b>レストラン名札</b> 인쇄.<h4>계산·판정</h4><ul><li>식수=숙소 그룹별 규칙. <b>석식=그날 묵는 전원</b>.</li><li><b>조기 퇴실 반영</b>: 방배정에서 🛫 조기 퇴실 처리한 인원은 끼니 경계대로 자동 차감 — <b>퇴실일 아침까지는 포함</b>, 점심·저녁부터 제외. 인쇄 명단·합계에 <code>早期退室 −N</code> 표기.</li><li><b>レストラン名札 = 운영팀 단위 합석</b>: 現地手配書에서 묶은 운영팀(team_group)이 <b>명패 1장으로 자동 합석</b>(대표자·태그 모두 표기·인원 합산). 같은 운영팀 중 <b>석식만 따로</b> 낼 팀은 「✂ 석식 분리」로 단독 명패로 뺀다(「↩ 합석 복원」 되돌림). 식수 집계는 그대로(각 팀 인원 유지).</li><li><b>제외(병합)</b>: 한 팀 지우고 다른 팀에 인원 취합 → 명단·식수에서 빠짐(묶기와 달리 명단에서 사라짐).</li></ul><h4>別注 → 정산 자동청구</h4>단가가 있는 <b>추가·업그레이드 別注를 등록하면 그 팀 御請求書(정산)에 자동으로 청구</b>가 잡힙니다(업그레이드=차액, 알레르기=청구 없음). 별주를 수정·삭제하면 청구도 자동으로 따라갑니다(DB 트리거). 팀 정산 계정이 없으면 자동 개설됩니다.<h4>주요 용어</h4>태그코드. レストラン名札은 운영팀(現地手配書) 단위 합석 · 「석식 분리」로 운영팀 중 그 팀만 단독 명패.<h4>석식 장소(날짜별)</h4>레스토랑/컨벤션홀 중 한 곳을 날짜마다 지정(시간·한 줄 안내 선택). <b>안내 모니터</b>(로비·2층 TV, signage.html — 로그인 없이 오늘·내일만 표시)와 夕食オーダー 인쇄 머리글에 나온다. 장소를 안 정한 날은 모니터에 「프론트에 문의」로 뜬다.<h4>권한·데이터</h4>print(인쇄) 영역. 태그·인원·묶기 저장은 print 또는 room 권한 필요. 석식 장소(dinner_venue)는 print·front·kitchen. 別注 청구는 charges(트리거가 생성, 정산은 settle/pos 영역에서 열람).',
    'shizu.html': '<h4>이 화면이 하는 일</h4><b>志津の宿 予約表</b>(시즈노야도 료칸 객실 예약표)를 만들고 <b>객실 배정 보드</b>로 개인 단위 배정합니다. 7객실=本館 4(志津·合歓·北條·山法師)·別棟 3(吉祥·瑞雲·馬酔木).<h4>배정</h4><ul><li><b>객실 배정 보드</b>: 왼쪽 미배정에서 인원 선택 → 오른쪽 객실 클릭으로 배정. 이름 클릭=이동·✕=미배정·드래그도 가능. <b>배정·이동·해제·전체비우기·자동배정은 모두 직후 <u>↩ 되돌리기</u></b>(10초 토스트)로 전단계 복원. 여러 명 이동 시 정원(3)에 막힌 인원은 <b>건너뜀 N</b>으로 알리며, 그대로 섞였다면 ↩로 되돌리세요. <b>검색</b>(이름·영문명·대표자·행사번호)으로 찾아 그 날짜로 점프.</li><li><b>🪄 월별 재배치</b>: 표시 중인 <b>그 달만</b> 자동배정을 풀고 다시 짠다(다른 달 무영향, 수기 보존, ↩ 되돌리기). <b>🪄 미배정만 배정</b>은 빈 자리에 넣기만 하고 기존 배정을 안 움직인다(step1 등록 때도 같은 규칙으로 자동 실행). <b>체류가 긴 팀부터</b>(짧은 체류가 긴 체류의 자리를 막지 않게) 2명씩 빈 방에 넣되, 같은 등급 안에서는 <b>앞뒤 손님과 틈이 가장 작은 방</b>을 고른다. <b>같은 팀이 이미 쓰는 방에 빈 침대가 있으면 먼저 합류</b>(남의 팀 방에는 안 섞음). 비고에 <b>트리플·3인 1실</b>이 있는 3명 팀은 한 방에 3명(트윈이라 「초과」 표시는 그대로 남음). 그 외 홀수 1명은 같은 팀 방 빈 침대에만, 없으면 미배정. 보드 머리의 <b>「필요 침대 / 보유」</b>로 부족한 밤을 먼저 본다. <b>등급 순서 — 일반팀: 本館 201·202·204(志津·合歓·山法師) → 203(北條) → 離れ 301·302·303(吉祥·瑞雲·馬酔木). 別棟 선호팀: 離れ(吉祥·瑞雲·馬酔木) 먼저→本館→北條. 北條(203)는 완전 최후가 아니라 <u>本館 중 후순위</u>(別棟보다 먼저)</b>.<br><b>고령자 우선</b> — 팀 처리 순서가 <b>팀 최고령 나이 내림차순</b>(동률이면 예약순)이라, 어르신이 있는 팀이 계단 적은 <b>本館(2층)</b>을 먼저 가져가고 젊은 팀이 離れ로 밀립니다. 나이=그 팀 <b>체크인일 기준 만나이</b>의 팀 내 최댓값(생년월일 없으면 맨 뒤). ⚠ <b>別棟 선호팀은 나이와 무관하게 離れ 우선</b> — 사전 요청이 나이보다 앞섭니다.</li><li><b>정원 2 + 강제 1명</b>: 수기로 최대 3인까지. 3인 방은 「3/2 超過」로 붉게 표시.</li><li><b>번들예약</b>(시즈+야마나미·시즈+간지 등 한 예약)은 예약기간 전체가 아니라 <b>실제 시즈 숙박 구간만</b> 미배정(未) 계산(현지비고 「M/D-M/D 시즈」 파싱·배정된 방 기준). 미배정 목록은 <b>숙소가 시즈인 팀 + 상품명에 시즈가 든 번들예약 팀</b>을 함께 봅니다 — 번들은 숙소가 야마나미/간지로 기록돼 있어, 이게 없으면 <b>배정을 푸는 순간 목록에서 사라져</b> 다시 배정할 수 없었습니다.</li><li><b>콤보 상품(시즈+간지·야마나미)의 시즈 숙박 기간</b> — 예약 기간(한국 출발~귀국)이 시즈 숙박보다 길므로, 현지비고의 「9/9-16일 … (시즈노야도)」「8/12-8/19 시즈」를 읽어 그 구간만 시즈로 봅니다(서버 자동채움도 같은 규칙). 비고가 없거나 형식이 달라 予約表에 예약 기간 전부가 떠 있으면, 배정 보드 위 팀 요약 칩의 <b>[기간]</b> 버튼으로 이 팀의 시즈 숙박 기간을 직접 고칩니다(예약 기간 안에서, 같은 방 그대로, 되돌리기 가능).</li></ul><h4>메모·표기</h4><ul><li><b>参考事項</b>=<b>현장 직접입력</b>(트리플룸·합팀 등 현장 필요한 것만 일본어로, 방 칸 클릭·팀 단위 자동 저장). ※한국 현지비고 자동표시는 폐지(한국어라 현장 무의미) — 긴 특이사항은 담당자에게 직접 전달. <b>現地메모</b>=방 칸 클릭해 직접 입력(팀 단위·자동 저장).</li><li><b>♨ 별채(내탕)</b>: 현지비고에 별채신청·내탕신청·온천신청 등 키워드가 있으면 別棟 방에 ♨ 자동 표기. 사전신청=<b>+2,000엔/인·박=19,000엔</b>(별채 3방·메리트 B2B).</li><li><b>現地예약 구분</b>: 메리트 B2B가 아닌 <b>현장 워크인 예약</b>은 <b>「現地」 배지</b>로 표시(배정보드·미리보기·인쇄 공통) — 현장에서 한눈에 구분.</li><li><b>객실 폐쇄 기간</b>: 배정 보드의 객실 카드에 <b>그 방의 폐쇄 기간이 늘 보입니다</b>(2026-09-15) — 보고 있는 날짜에 걸린 폐쇄는 빨강 「🔒 폐쇄 M/D~M/D · 사유」, 앞으로 예정된 폐쇄는 주황 「🔒 폐쇄 기간 M/D~M/D · 사유」. 이미 끝난 폐쇄는 안 보입니다.</li><li>날짜 칩에 <b>入N/出N</b>(체크인/체크아웃 팀)·<b>未N</b>(미배정). 월 선택↔보드 사이에 그날 <b>체크인/체크아웃 팀 요약</b>.</li><li><b>📌 현지 전달사항</b>(체크인/체크아웃 목록 바로 아래): 그 날짜에 묵는 팀 중 <b>현장이 미리 알아야 할 게 있는 팀만</b> 모아 보여줍니다. 자동 배지 — <b>콤보 체류</b>(시즈+간지·시즈+야마나미 등 복수 숙박지)·<b>별채 신청</b>·<b>트리플</b>·<b>식사·알러지</b>·<b>기타</b>. 소스=엠클릭 현지비고·비고 + 현장 입력(참고사항·現地메모) + 상품명. <b>원문을 그대로 함께 표시</b>하므로 배지는 눈에 띄게 하는 용도이고 판단은 원문을 보고 하십시오.</li></ul><h4>출력</h4>화면 예약표를 <b>그대로 인쇄</b>(팝업 없음·2일/장·큰 글씨)·날짜별 인쇄. 全角 표기.<h4>권한·데이터</h4><b>shizu 영역</b>(admin.html에서 지정). rooms·guest_members·passengers 읽기 + shizu_team_memo(現地메모)·shizu_memo 쓰기(shizu/room 영역).',
    ...
    'audit.html': '<h4>이 화면이 하는 일</h4>데이터 <b>정합성 이상</b>을 인쇄·실사용 전에 한 번에 점검합니다. <b>마스터(admin) 전용</b>. [재검수]로 실행 → 항목별 건수·표본(행사번호)을 보여줍니다.<h4>점검 항목</h4><ul><li><b>개인번호 결번</b>: 네임택 개인번호가 팀 내 1..N 연속이 아님(과거 임포트 잔재). 인쇄는 자동 재부여로 정상, 재임포트하면 DB도 교정.</li><li><b>명단 수 불일치</b>: guest_members ≠ passengers 인원.</li><li><b>팀 내 태그 중복</b> / <b>연결 끊긴 방배정</b>(FK 끊김) / <b>방 정원 초과</b>(더블부킹).</li></ul><h4>통계 검산 (2026-09 추가)</h4>가동률·오늘 매출·현장 매출·B2B 청구가 <b>조용히 틀려 있던</b> 일이 있었습니다 — 화면은 멀쩡히 숫자를 보여주니 틀린 줄 모릅니다. 그래서 대표 숫자마다 <b>다른 모양의 두 번째 계산</b>을 두고 [재검수] 때 서로 맞춰 봅니다.<ul><li><b>예약 인원 ≠ 명단</b> / <b>체류 박수 ≠ 상품명 「N박」</b> — B2B 청구의 근거 둘이 어긋나면 과소·과다 청구의 씨앗.</li><li><b>미래 날짜 청구</b> — 카트처럼 예정으로 잡히는 것. 이상이 아니라 「현장 매출」을 읽을 때의 참고.</li><li><b>대기 예약에 방·청구가 붙음</b> — 대기는 현장에 오지 않으므로 붙어 있으면 상태가 어긋난 것.</li><li><b>가동률 검산</b> · <b>매출 검산</b> — 같은 숫자를 두 방식으로 계산해 비교. <b>다르면 계산식 버그</b>이니 그 숫자를 믿지 말고 알려 주세요.</li></ul><b>규칙</b>: 앞으로 새 집계를 만들 때는 반드시 다른 모양의 쿼리로 한 번 더 계산해 대조하고, 그 대조를 여기에 넣습니다.<h4>정상값</h4>연결 끊김·정원초과·검산 불일치·대기 누수는 <b>0이 정상</b>. 결번·불일치는 재임포트로 교정.<h4>권한·데이터</h4>admin 전용. RPC <code>data_audit</code>(서버 집계, security definer·is_admin 가드). 읽기 전용(데이터 변경 없음).',

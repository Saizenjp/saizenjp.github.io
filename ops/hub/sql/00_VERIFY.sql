...
from (
  ...
  union all select 141,'141 시즈 서버 자동채움','_shizu_autofill_impl 이 화면 규칙(트리플 3명·같은 팀 합류·best-fit)인가', case when exists(select 1 from pg_proc where proname='_shizu_autofill_impl' and prosrc like '%TRIPLE_RE%' and prosrc like '%v_gb%') then '✅ 적용' else '🔴 미적용 (141) — step1 등록 때 옛 규칙(2인 페어만)으로 채움' end
  union all select 142,'142 시즈 콤보 구간','서버 자동채움이 콤보(시즈+간지·야마나미) 팀의 실제 시즈 구간(_shizu_span)을 쓰는가', case when exists(select 1 from pg_proc where proname='_shizu_span') and exists(select 1 from pg_proc where proname='_shizu_autofill_impl' and prosrc like '%_shizu_span%') then '✅ 적용' else '🔴 미적용 (142) — 콤보 팀이 予約表에 예약기간 전부로 뜸' end
  union all select 140,'140 안내 모니터 영역','signage 가 부모 그룹(sign_lobby 등 4자식)이고 print 자식이 아닌가', case when (select count(*) from area_tree where parent='signage')=4 and not exists(select 1 from area_tree where child='signage') then '✅ 분리' else '🔴 미적용 (140) — 모니터 카드가 인쇄 그룹에 섞임' end
  ...
  union all select 138,'138 권한 세부 영역','area_tree 표(자식→부모) 36행(140 이후)', case when (select count(*) from area_tree) = 36 then '✅ 36행' else '❌ 행수 다름 (138/140) — saizen-core AREA_TREE 와 대조' end

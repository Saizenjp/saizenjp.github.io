...
union all select '11 menu_items (메뉴 마스터)',   case when to_regclass('public.menu_items')              is not null then '✅ 있음' else '❌ 없음 (11 실행)' end
union all select '12 menu_items.station 컬럼',    case when exists(select 1 from information_schema.columns where table_schema='public' and table_name='menu_items' and column_name='station') then '✅ 있음' else '❌ 없음 (12 실행)' end
union all select '12 kitchen_tickets (주방큐)',   case when to_regclass('public.kitchen_tickets')         is not null then '✅ 있음' else '❌ 없음 (12 실행)' end
order by 항목;

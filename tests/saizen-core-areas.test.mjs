...
test('일별 체류 현황(staycal)만 있으면 경영통계(stats_mgmt)는 못 연다', () => {
...
test('SQL 138 seed 와 AREA_TREE 가 같다', () => {
  const sql = readFileSync(new URL('../ops/hub/sql/138_area_tree.sql', import.meta.url), 'utf8');
  ...
  assert.equal(Object.keys(mine).length, 33);
});

...
/ **정식 사이트 기술 보강**(파비콘=SaiZen 세로로고(`/assets/logo-saizen-vertical.svg`)·theme-color `#647548` — saizen-ops.js `mountHead()`가 전 ops 페이지 주입 + 루트/ops 랜딩엔 static. **robots noindex+`robots.txt` Disallow /**(고객 PII 취급 내부도구라 검색색인 차단). **루트 `404.html`**(브랜드 안내→`/ops/`). description 메타. saizen-ops `?v=14.60`. ※개인정보처리방침·이용약관·문의처는 Min 정보·법적 검토 필요 → 보류).

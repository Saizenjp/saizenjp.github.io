...
/ **夕食 끼니 제외 토글**(화면 전용. 명단표 각 팀 행에 朝·昼·夕 칩(기본 포함, 클릭=제외, 회색 취소선). `manualData[dinner_${date}_${seq}].exB/exL/exD`. 夕食オーダー 합계 반영: exD=인쇄 명단·석식서 제외 / exB=翌朝 조식(전날밤 행 키) / exL=昼食(귀국=전날밤·그외=당일 행 키). 인쇄엔 토글 없음, 합계만).

...
- **배정 UX 개선(2026-06)**: ① **미배정 목록을 선택 날짜 기준으로**(`teamInDayScope` — 카드=focus 하루 점유 / 타임라인=선택 일자) → 객실 카드와 같은 날짜만 보여 "어느 날짜에 배정하는지" 명확 + 목록 짧아짐. 헤더에 `dayScopeLabel()` 날짜 표기. ② **✕ 해제 되돌리기**(`toastUndo`(빨강)+`restoreRooms`+`_roomSnap` — 해제 직후 6초 토스트의 [↩ 되돌리기]로 같은 방·기간 즉시 복구. 칩 ✕=`unassign`, 명단 ✕해제=`unassignMember`가 분할 다건도 모아 1회 undo. ※자동연동 추가요금은 복구 대상 아님). ③ **빈 방 클릭 → 배정 피커**(`openAssignPicker` — 선택 인원 없을 때 방 클릭 시 잔소리 토스트 대신 팝업: 그 날짜 점유·미배정 인원을 팀별 체크박스로, 같은 숙소 팀 우선정렬·정원 표기, 선택→assignMembers로 배정). ④ **객실 사용/잠금=🔓/🔒 아이콘 토글**.

...
/ **夕食 끼니 제외 체크**(화면 전용. 명단표 각 팀 행에 「食事除外」 + 朝·昼·夕 **체크박스**(체크=제외). 제외된 끼니만 **빨강 뱃지+「除外」** 로 또렷이 표시(흐릿 취소선·숨김 스타일 폐기 — Min 피드백). `manualData[dinner_${date}_${seq}].exB/exL/exD`. 夕食オーダー 합계 반영: exD=인쇄 명단·석식서 제외 / exB=翌朝 조식(전날밤 행 키) / exL=昼食(귀국=전날밤·그외=당일 행 키). 인쇄엔 토글 없음, 합계만).

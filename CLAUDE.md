...
- `saizen-ops.js/css` 변경 시 **모든 Hub 페이지의 `?v=` 캐시 버전을 올린다**(현재 `14.5`).
...
- **방배정 = 개인 단위**(4인 그룹이 트윈 2개로 쪼개지는 현실). **디럭스급(디럭스더블트윈·디럭스트윈)은 회원권 보유자 우선** — 비회원은 일반 트윈·컴팩트트윈, 디럭스에 비회원 수동배정 시 **소프트 경고**(막지 않음). 현장은 트윈·컴팩트트윈을 사실상 동급으로 보고 디럭스만 프리미엄으로 인식. *(설계 규칙 — 현재 `room.html`은 디럭스 객실명만 존재하고 회원권 판정·💎 우선배정·소프트 경고는 미구현.)*
...
---
*최종 검증 시점: app v14.5 / Hub asset `?v=14.5` / SQL 01~08 / 탭 ①~⑥ 활성·⑦~⑨ 준비중.
